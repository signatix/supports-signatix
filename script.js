document.addEventListener('DOMContentLoaded', () => {
  ...
  const showResults = (usage) => {
    ...
    const cardsHTML = results.map(rec => {
      const vignetteUrl = (typeof vignettesProduits !== 'undefined' && vignettesProduits[rec.url]) ? vignettesProduits[rec.url] : null;
      return `
        <li class="bg-white rounded-xl border border-slate-200 p-6 flex flex-col hover:shadow-lg hover:border-pink-300 transition duration-300 fade-in">
          ${vignetteUrl ? `<img src="${vignetteUrl}" alt="Vignette ${rec.nom}" class="mb-4 w-full h-40 object-contain rounded-lg border" />` : ''}
